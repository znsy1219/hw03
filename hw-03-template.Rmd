...
a) Build a workflow for the training data that consists of a recipe (`gss16_rec_1`) and a model (`gss16_spec_1`). Name this workflow `gss16_wflow_1`.
...
a) Build a new workflow for the training data that consists of a recipe (`gss16_rec_2`) and a model (`gss16_spec_2`). Name this workflow `gss16_wflow_2`.
    
Now the new recipe (named `gss16_rec_2`) should contain the following steps for predicting `advfront` from `polviews`, `wrkstat`, and `educ`:

  - `step_dummy()` to create dummy variables for `all_nominal()` variables that are predictors, i.e. not "outcomes". You can select outcomes using `all_outcomes()`

  - The model (named `gss16_spec_2`) should specify a model that is appropriate for the data as well as the computational engine.
  
Apply the workflow you defined earlier to the training dataset and create a new model fit named as `gss16_fit_2`. Then use the fitted models to predict the test data, plot the ROC curves for the predictions for both models, and calculate the areas under the ROC curves.

```{r}

```

b) Now, consider another data splitting idea (80\% training and 20\% testing, named as `gss16_split_2`) for the same workflow `gss16_wflow_2`: 

- Again predict `advfront` from `polviews`, `wrkstat`, and `educ`. 

- Update your data splitting step first, workflow and recipe to reflect this new specification (and name the updated data split `gss16_split_2`). 

Apply your `gss16_wflow_2` workflow to the new training data using the workflow `gss16_wflow_2` with different data split (`gss16_split_2`) that you have defined. 
 
Comment on which model performs better 
  * the model only including `educ`, model 1; 
  * the model including `polviews`, `wrkstat`, and `educ` with `gss16_split` as model 2 or
  * the model including `polviews`, `wrkstat`, and `educ` with `gss16_split_2` as model 3. 
  
Explain your reasoning. Discuss the potential difference for the model performance under two different data splits (`gss16_split` vs `gss16_split_2`)
